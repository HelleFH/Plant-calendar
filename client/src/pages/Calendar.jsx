...
// Function to fetch listings by date
const getListingsByDate = async (date) => {
  try {
    const response = await axios.get(`http://localhost:3001/api/v1/listings/${date}`);
    return response.data;
  } catch (error) {
    // Check if the error is a 404 Not Found error
    if (error.response && error.response.status === 404) {
      // No listings found for the date, return an empty array
      return [];
    } else {
...
const CalendarComponent = () => {
    ...
    const fetchAndSaveListingDates = async () => {
      try {
        ...
        for (const date of datesInRange) {
          ...
          const listingsData = await getListingsByDate(formattedDate);
  ...
  const fetchListings = async (date) => {
    try {
      // Call getListingsByDate function to fetch listings for the selected date
      const formattedDate = date.toISOString().split('T')[0];
      const listingsData = await getListingsByDate(formattedDate);
